            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (login != null && login.Length > 0)
                    {
                        ...
                        var user = userModel.Login(login, pass, is_cc).user;
                        ...
                        if ((user.email.Trim().Length > 0) && m_EmailHelper.IsValidEmail(user.email.Trim()))
                        {
                            ...
                            emailNotificationModel.SaveEmailBeforeSend(0, user.id, user.company_id, user.email.Trim(), ConfigurationManager.AppSettings["emailFrom"], "",
        ...
        public string CreateUser(string code, string first, string last, string email, string title, int currentDepartmens, int currentLocations)
        {
            ...
            if (login != null && login.Length > 0)
            {
                ...
                if ((user.email.Trim().Length > 0) && m_EmailHelper.IsValidEmail(user.email.Trim()))
                {
                    ...
                    emailNotificationModel.SaveEmailBeforeSend(0, _user.id, _user.company_id, user.email.Trim(), ConfigurationManager.AppSettings["emailFrom"], "",
                    ...
                    foreach (user _user in cm.AllSupervisingMediators(cm._company.id, true))
                    {
                        if ((_user.email.Trim().Length > 0) && m_EmailHelper.IsValidEmail(_user.email.Trim()))
                        {
                            ...
                            emailNotificationModel.SaveEmailBeforeSend(0, _user.id, _user.company_id, _user.email.Trim(), ConfigurationManager.AppSettings["emailFrom"], "",
