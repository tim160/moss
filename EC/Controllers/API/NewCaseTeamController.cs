        ...
        public object Post(Filter filter)
        {
            ...
            if (filter.AddToTeam.HasValue)
            {
                ...
                if ((_um._user.email.Trim().Length > 0) && m_EmailHelper.IsValidEmail(_um._user.email.Trim()))
                {
                    ...
                    string body = eb.Body;
                    glb.SaveEmailBeforeSend(_um._user.id, _um._user.company_id, _um._user.email.Trim(), System.Configuration.ConfigurationManager.AppSettings["emailFrom"], "", LocalizationGetter.GetString("Email_Title_MediatorAssigned", is_cc), body, false, 40);
            ...
            if (filter.MakeCaseOwner.HasValue)
            {
                ...
                string body = eb.Body;
                glb.SaveEmailBeforeSend(_um._user.id, _um._user.company_id, _um._user.email.Trim(), System.Configuration.ConfigurationManager.AppSettings["emailFrom"], "", LocalizationGetter.GetString("Email_Title_SetCaseOwner", is_cc), body, false, 65);
