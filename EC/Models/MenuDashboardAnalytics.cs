...
namespace EC.Models
{
    public class MenuDashboardAnalytics
    {
        ...
        //public object getResultObjByCompanies(int[] companyArray)
        //{
        //    1) можно взять из глобальной функции методы получения департаментов и тпе далее.
        //        2) может даже их на фронте сделать. данные те же приходят.
        //        3) переделать отправку фильтров. то мы по id отправляли, можно теперь отправить по имени Ukraine например. там надо посмотреть.
        //        хотя с другой стороны, можно и это сделать на фронте - берешь существующий массив, исключаешь из него данные. и отдаещь фронт
