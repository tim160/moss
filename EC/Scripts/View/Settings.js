$(document).ready(function () {
    ...
    */
    ...
    $(".resendInvitation").on('click', function (event) {

        var currentItem = $(event.currentTarget);
        var email = currentItem.parent('.blocksInfo').find('.emailBlockInfo').text();

        if (email != "" || email != undefined) {
            $.ajax({
                method: "POST",
                url: "/Settings/resendInvitation",
                data: {
                    email: email.trim(),
                }
            }).done(function (data) {//data from server
